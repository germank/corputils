    ...
    for line in fileinput.input(args.corpora):
        ...
        if len(t) == 1 and line.strip('\n<>') == "/{0}".format(args.separator):
            try:
                ...
                logging.exception("Failed while processing sentence:"
                "\n{0}".format("\n".join(plain_text_sentence)))
            finally:
                #start a new sentence
                sentence = []
                plain_text_sentence = []
                token_id = 0
        elif len(t) == 1 and line.startswith('<'):
            ...
            #ad-hoc fix for badly numerated sentences
            while token_id < t[3]:
                fake_tuple = ["<fake>", "<fake>", "FK", token_id, token_id,
                "<FAKE>", "<FAKE>", "<FAKE>"]
                sentence.append(tuple(fake_tuple))
                plain_text_sentence.append("<FAKE FAKE FAKE FAKE>")
                token_id +=1
